...
describe('Layer 2 capability bundles', () => {
	...
	it('navigableCollectionCapability composes collection, roving, and navigation', () => {
		...
		}).find((cap): cap is CollectionCapability<unknown> => cap.slot === collectionSlot('item'))!;
	...
	it('typeaheadCollectionCapability composes input and buffered search without claiming to filter', () => {
		let query = $state('');
		...
		const caps = typeaheadCollectionCapability({
			kind: 'item',
			input,
			roving,
		...
		const collection = bond.requireSurface(collectionSlot('item'));
		collection.set('rust', { label: 'Rust' });
		const container = addAtom(bond, 'list', 'container');

		expect(caps.meta).toMatchObject({ layer: 2, kind: 'focused' });
		expect(bond.capability(TYPEAHEAD_COLLECTION)).toBeDefined();
		expect(bond.capability(INPUT)?.surface).toBe(input);
		expect(bond.capability(TYPEAHEAD)?.surface?.search('ru')).toBe('rust');
		expect(container.spread['aria-orientation']).toBe('vertical');
		input.set('ru');
		expect(collection.keys).toEqual(['rust']);
		unmount();
	});
