...
describe('Combobox callbacks', () => {
	it('reports committed open, selection, and query transitions without initialization echoes', () => {
		const onopenchange = vi.fn((value: boolean, { bond }: { bond?: ComboboxBond }) => {
			expect(bond?.props.open).toBe(value);
		});
		const onvaluechange = vi.fn((value: unknown, { bond }: { bond?: ComboboxBond }) => {
			expect(bond?.props.values?.[0]).toBe(value);
		});
		const onvalueschange = vi.fn((value: unknown[], { bond }: { bond?: ComboboxBond }) => {
			expect(bond?.props.values).toEqual(value);
		});
		const onquerychange = vi.fn((value: string, { bond }: { bond?: ComboboxBond }) => {
			expect(bond?.props.query).toBe(value);
		});
		const { component } = render(ComboboxRoot, {
			onopenchange,
			onvaluechange,
			onvalueschange,
			onquerychange
		});
		const bond = (component as unknown as { getBond(): ComboboxBond }).getBond();
	...
	it('does not report parent-controlled prop echoes', async () => {
		...
		const { component, rerender } = render(ComboboxRoot, {
			onopenchange,
			onvaluechange,
			onvalueschange,
			onquerychange
