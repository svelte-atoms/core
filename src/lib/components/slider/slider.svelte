...
>
	...
	>
		...
		<HtmlAtom
			...
			as="div"
			class={[
				'slider-fill bg-foreground absolute rounded-full transition-none',
	...
	<input
		...
		class={[
			...
			isVertical && 'slider-input--vertical'
		].filter(Boolean).join(' ')}
		aria-valuemin={min}
		aria-valuemax={max}
		aria-valuenow={value}
	...
	<HtmlAtom
		...
		as="div"
		class={[
			'slider-thumb bg-background border-border pointer-events-none absolute h-5 w-5 rounded-full border-2 shadow-sm transition-none',
			isVertical
