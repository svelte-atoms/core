...
<script lang="ts">
	...
	const plannerEvents = new Map<number, PlannerEvent[]>([
		...
		[startOfDay(addDays(today, 1)).getTime(), [
			{ title: 'Client demo · 2pm', tone: 'bg-emerald-500/10 text-emerald-600 dark:text-emerald-400' }
		]],
...
{#snippet monthNavHeader(pivote: Date, onPrev: () => void, onNext: () => void)}
	<div class="flex items-center justify-between px-4 py-3 border-b border-border">
		<button
			...
			onclick={onPrev}
		>
			<svg viewBox="0 0 24 24" class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2.5"><path d="M15 18l-6-6 6-6" stroke-linecap="round" stroke-linejoin="round"/></svg>
		</button>
		...
		<button
			...
			onclick={onNext}
		>
			<svg viewBox="0 0 24 24" class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2.5"><path d="M9 18l6-6-6-6" stroke-linecap="round" stroke-linejoin="round"/></svg>
		</button>
	</div>
{/snippet}
			...
			{#if args.type === 'range'}
				<div class="flex items-center gap-2 text-sm mt-4">
					<div class="flex flex-col items-center gap-1">
						<span class="text-[10px] text-muted-foreground uppercase tracking-widest font-medium">Start</span>
						<span class="px-4 py-2 rounded-lg border border-border bg-muted/50 font-medium min-w-32 text-center">
							{defaultRange[0] ? format(defaultRange[0], 'MMM d, yyyy') : '—'}
						</span>
					</div>
					<svg viewBox="0 0 24 24" class="w-4 h-4 text-muted-foreground mt-5" fill="none" stroke="currentColor" stroke-width="2"><path d="M5 12h14M12 5l7 7-7 7" stroke-linecap="round" stroke-linejoin="round"/></svg>
					<div class="flex flex-col items-center gap-1">
						<span class="text-[10px] text-muted-foreground uppercase tracking-widest font-medium">End</span>
						<span class="px-4 py-2 rounded-lg border border-border bg-muted/50 font-medium min-w-32 text-center">
	...
	<div class="flex flex-col items-center justify-center p-8 min-h-screen gap-4">
		...
		{#if rangePicker[0] && rangePicker[1]}
			<div class="flex items-center gap-2">
				...
				<button class="text-xs text-muted-foreground hover:text-foreground transition-colors" onclick={() => { rangePicker = [undefined, undefined]; }}>
					Clear
				</button>
			</div>
		{/if}
...
<Story name="Restricted Dates">
	<div class="flex flex-col items-center justify-center p-8 min-h-screen gap-4">
		...
		<div class="flex items-center gap-2 text-xs text-muted-foreground bg-muted/60 px-3 py-2 rounded-lg border border-border">
			<svg viewBox="0 0 24 24" class="w-3.5 h-3.5 shrink-0" fill="none" stroke="currentColor" stroke-width="2"><rect x="3" y="4" width="18" height="18" rx="2"/><path d="M16 2v4M8 2v4M3 10h18"/></svg>
...
<Story name="With Events">
	<div class="flex flex-col items-center justify-center p-8 min-h-screen gap-4">
								...
								{#snippet children({ calendar })}
									<div class="flex items-center justify-center size-full">
										{#if eventDays.has(startOfDay(day.date).getTime()) && !calendar.state.isDaySelected(day)}
											<span class="text-[0.82em] leading-none underline underline-offset-2 decoration-primary/70 decoration-[1.5px]">{day.dayOfMonth}</span>
										{:else}
		...
		<div class="flex items-center gap-2 text-xs text-muted-foreground">
			<span class="text-[0.85em] underline underline-offset-2 decoration-primary/70 decoration-[1.5px]">7</span>
	...
	<div class="flex flex-col items-center justify-center p-8 min-h-screen gap-5">
		<div class="rounded-xl border border-border bg-background shadow-lg overflow-hidden">
			<div class="flex items-center px-5 py-3 border-b border-border gap-4">
				...
				<span class="text-sm font-semibold flex-1 text-center">{format(twoMonthPivote, 'MMMM yyyy')}</span>
				<div class="w-px h-4 bg-border"></div>
				<span class="text-sm font-semibold flex-1 text-center">{format(twoMonthPivote2, 'MMMM yyyy')}</span>
				<button
					aria-label="Next month"
					class="flex items-center justify-center w-7 h-7 rounded-md hover:bg-accent transition-colors text-muted-foreground hover:text-foreground"
		...
		<div class="flex items-center gap-2 text-sm">
			...
			{#if twoMonthRange[0] && twoMonthRange[1]}
				<div class="flex flex-col items-center gap-1">
					<span class="text-[10px] text-muted-foreground uppercase tracking-widest font-medium">Nights</span>
					<span class="px-4 py-2 rounded-lg border border-primary/40 bg-primary/5 font-semibold min-w-16 text-center text-primary">
		...
		{#if twoMonthRange[0] && twoMonthRange[1]}
			<button class="text-xs text-muted-foreground hover:text-foreground transition-colors" onclick={() => { twoMonthRange = [undefined, undefined]; }}>
				Clear selection
			</button>
		{/if}
	</div>
...
<Story name="Booking Widget">
	<div class="flex flex-col items-center justify-center p-8 min-h-screen">
		<div class="w-96 rounded-2xl border border-border bg-background shadow-xl overflow-hidden">
			...
			<div class="px-1 pb-1">
				<CalendarModule.Root bind:pivote={bookingPivote} bind:range={bookingRange} type="range" min={today}>
					<CalendarModule.Header />
					<CalendarModule.Body weekday={undefined}>
						{#snippet children({ day }: { day: Day })}
							<CalendarModule.Day {day} />
			...
			<div class="border-t border-border px-5 py-4">
				<div class="flex gap-3 mb-4">
					<div class="flex-1 rounded-lg border border-border px-3 py-2.5">
						<div class="text-[10px] text-muted-foreground uppercase tracking-widest font-medium mb-0.5">Check-in</div>
						<div class="text-sm font-semibold">{bookingRange[0] ? format(bookingRange[0], 'MMM d') : '—'}</div>
					</div>
					<div class="flex-1 rounded-lg border border-border px-3 py-2.5">
						<div class="text-[10px] text-muted-foreground uppercase tracking-widest font-medium mb-0.5">Check-out</div>
						<div class="text-sm font-semibold">{bookingRange[1] ? format(bookingRange[1], 'MMM d') : '—'}</div>
					</div>
				</div>
				...
				<div class="flex gap-2">
					<button
						...
						onclick={() => { bookingRange = [undefined, undefined]; }}
					>
						Clear
					</button>
					<button
...
<Story name="Showcase">
	<div class="flex flex-col items-center justify-center p-10 min-h-screen gap-8 bg-muted/30">
		<div>
			...
			<p class="text-sm text-muted-foreground text-center mt-1">All states rendered simultaneously — useful for visual regression.</p>
		</div>

		<div class="flex flex-wrap justify-center gap-6">
			<!-- 1. Single select, today pre-selected -->
			<div class="flex flex-col gap-2">
				<span class="text-xs text-muted-foreground font-medium uppercase tracking-widest text-center">Single</span>
				<div class="w-72 rounded-xl border border-border bg-background shadow-md overflow-hidden">
					{@render monthNavHeader(
						...
						<CalendarModule.Root bind:pivote={showcasePivote1} bind:range={showcaseRange1} type="single">
							<CalendarModule.Header />
							<CalendarModule.Body weekday={undefined}>
								{#snippet children({ day }: { day: Day })}
									<CalendarModule.Day {day} />
			...
			<div class="flex flex-col gap-2">
				<span class="text-xs text-muted-foreground font-medium uppercase tracking-widest text-center">Range</span>
				<div class="w-72 rounded-xl border border-border bg-background shadow-md overflow-hidden">
					{@render monthNavHeader(
						showcasePivote2,
						...
						<CalendarModule.Root bind:pivote={showcasePivote2} bind:range={showcaseRange2} type="range">
							<CalendarModule.Header />
							<CalendarModule.Body weekday={undefined}>
								{#snippet children({ day }: { day: Day })}
									<CalendarModule.Day {day} />
			...
			<!-- 3. Event dots -->
			<div class="flex flex-col gap-2">
				<span class="text-xs text-muted-foreground font-medium uppercase tracking-widest text-center">Events</span>
				<div class="w-72 rounded-xl border border-border bg-background shadow-md overflow-hidden">
					{@render monthNavHeader(
						showcasePivote3,
	...
	<div class="flex flex-col items-center justify-center p-8 min-h-screen">
		<div class="w-80 flex flex-col gap-5">
			...
			<div class="flex flex-col gap-1.5">
				...
				>
					<svg viewBox="0 0 24 24" class="w-4 h-4 text-muted-foreground shrink-0" fill="none" stroke="currentColor" stroke-width="2"><rect x="3" y="4" width="18" height="18" rx="2"/><path d="M16 2v4M8 2v4M3 10h18"/></svg>
					{#if inputPickerDate}
						...
						<button
							...
							onclick={(e) => { e.stopPropagation(); inputPickerRange = [undefined, undefined]; }}
						>
							<svg viewBox="0 0 24 24" class="w-3.5 h-3.5" fill="none" stroke="currentColor" stroke-width="2.5"><path d="M18 6 6 18M6 6l12 12" stroke-linecap="round"/></svg>
						</button>
					{:else}
			...
			<div class="rounded-xl border border-border bg-background shadow-lg overflow-hidden">
				{@render monthNavHeader(
					...
					<CalendarModule.Root bind:pivote={inputPickerPivote} bind:range={inputPickerRange} type="single">
						<CalendarModule.Header />
						<CalendarModule.Body weekday={undefined}>
							{#snippet children({ day }: { day: Day })}
								<CalendarModule.Day {day} />
	...
	<div class="flex flex-col items-center justify-center p-8 min-h-screen gap-4">
		...
		{#if weekRange[0] && weekRange[1]}
			<div class="flex items-center gap-3">
				<span class="px-3 py-1.5 rounded-lg bg-primary/10 text-primary text-sm font-medium border border-primary/20">
					{format(weekRange[0], 'MMM d')} – {format(weekRange[1], 'MMM d, yyyy')}
				</span>
				<button
					class="text-xs text-muted-foreground hover:text-foreground transition-colors"
					onclick={() => { weekRange = [undefined, undefined]; }}
				>
					Clear
				</button>
			</div>
	...
	<div class="flex flex-col items-center justify-center p-8 min-h-screen gap-5">
		<div class="w-80 rounded-xl border border-border bg-background shadow-lg overflow-hidden">
			{@render monthNavHeader(
				...
				<CalendarModule.Root bind:pivote={pivoteAvailability} bind:range={availabilityRange} type="single" min={today}>
					<CalendarModule.Header />
					<CalendarModule.Body weekday={undefined}>
						{#snippet children({ day }: { day: Day })}
							...
							<CalendarModule.Day {day} class={avail === 'full' ? 'pointer-events-none' : ''}>
								{#snippet children({ calendar })}
									...
									<div class="flex flex-col items-center justify-center size-full gap-px">
										<span class={['leading-none text-[0.82em]', avail === 'full' && !day.offmonth ? 'opacity-40' : '']}>{day.dayOfMonth}</span>
										{#if avail === 'limited'}
											<span class={['text-[7px] font-semibold leading-none', selected ? 'text-primary-foreground/80' : 'text-amber-500']}>FEW LEFT</span>
										{:else if avail === 'full'}
											<span class="text-[7px] font-semibold leading-none text-destructive/60">FULL</span>
										{/if}
									</div>
								{/snippet}
							</CalendarModule.Day>
		...
		<div class="flex items-center gap-5 text-xs bg-muted/50 px-4 py-2.5 rounded-lg border border-border">
		...
		{#if availabilityRange[0]}
			<div class="flex items-center gap-3 text-sm">
				...
				<button
					class="text-xs text-muted-foreground hover:text-foreground transition-colors"
					onclick={() => { availabilityRange = [undefined, undefined]; }}
				>
					Clear
				</button>
			</div>
...
>
	{#snippet template(args: { type: 'single' | 'range' })}
		...
		{#snippet dayCell(day: Day)}
			<CalendarModule.Day {day} class="aspect-auto h-full flex p-0 hover:bg-transparent hover:text-foreground/80">
				{#snippet children({ calendar })}
					{@const selected = calendar.state.isDaySelected(day)}
					...
					>
						...
						<div class="flex flex-col gap-1 overflow-hidden">
							{#each events as event (event.title)}
								<span class={['flex items-center gap-1.5 rounded px-1.5 py-0.5 text-[11px] leading-tight', event.tone]}>
				...
				<div class="flex items-center gap-3">
					{#if args.type === 'range' && plannerRange[0]}
						<span class="text-sm text-muted-foreground">
							...
							{#if plannerRange[1]}
								→ {format(plannerRange[1], 'MMM d')} · {differenceInCalendarDays(plannerRange[1], plannerRange[0])}d
							{/if}
						</span>
					{/if}
					<button
						class="text-sm px-3 py-1.5 rounded-lg border border-border hover:bg-accent transition-colors"
						onclick={() => { pivotePlanner = new Date(); plannerRange = [undefined, undefined]; }}
					>
						Today
					</button>
				</div>
