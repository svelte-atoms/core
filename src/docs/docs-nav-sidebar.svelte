...
<Drawer.Root bind:open {onclose} class="lg:hidden">
	{#snippet children({ drawer })}
		...
		<Drawer.Content  side="left" class="bg-background border-border w-72 border-r p-0 shadow-xl pt-32" animate={animateDrawerContentFromLeft({duration: DURATION.smooth/1000})}>
