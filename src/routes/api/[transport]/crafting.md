...
Components in @svelte-atoms/core follow consistent file organization patterns. The structure differs between simple (single-file) and compound (multi-part) components.

**Important:** Bonds only exist for compound components with multiple atoms that need to share state. Simple single-file components do not require a bond.
...
├── popover-content.svelte   # Content sub-component
...
│   ├── PopoverBondState (open, close, toggle methods)
│   └── PopoverBond (context sharing & element prop generators)
├── popover-root.svelte
│   └── Creates bond, provides context via bond.share()
├── popover-trigger.svelte
│   └── Accesses bond via PopoverBond.get()
