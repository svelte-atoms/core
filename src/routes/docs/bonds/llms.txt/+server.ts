...
${metadata.architecture.map((arch) => `### ${arch.component}\n\n${arch.description}\n\n**Responsibilities:**\n${list(arch.responsibilities)}`).join('\n\n')}

## Creating a Bond

### Step 1: Define Props Type

${codeBlock(`import { BondStateProps } from '@svelte-atoms/core';
...
${metadata.bondPatterns.map((pattern) => `### ${pattern.title}\n\n${pattern.description}\n\n**Use Case:** ${pattern.useCase}`).join('\n\n')}
