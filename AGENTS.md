...
- Consult `docs/naming-convention.md`, `docs/philosophy.md`, `docs/components/*.md` when relevant.
...
vocabulary glossary; `src/lib/shared/*/README.md` records runtime binding decisions. When in
doubt, copy the nearest sibling component — consistency across ~50 components beats local cleverness.

### Component anatomy

Every multi-part component is a folder `src/lib/components/<name>/` with a fixed layout:

- `bond.svelte.ts` — the `Bond` subclass (via `defineBond`) and each slot's `Atom` classes. The brain.
- `<name>-root.svelte` — constructs the bond and `share()`s it to context.
- `<name>-<part>.svelte` — one presentational component per slot.
- `types.ts`, `atoms.ts`, `index.ts`, optional `motion.svelte.ts` / `attachments.svelte.ts`.
- `<name>.stories.svelte`.

Mirror this layout; don't invent a new file split.

### Presets: resolve in `<script>`, never inline

Fold props into one `$derived` in the script, then spread. SSOT helpers in
`src/lib/components/atom/utils/props.ts`:

- **Bonded part** → `mergeAtomProps(atom, preset, restProps)` = `{ preset: preset ?? atom?.preset, ...atom?.spread, ...restProps }`. See `collapsible-header.svelte`.
- **Static part** (no atom) → `mergePresetProps(preset, 'fallbackKey', restProps)` = `{ preset: preset ?? 'fallbackKey', ...restProps }`. See `button.svelte`.

```svelte
const buttonProps = $derived(mergePresetProps(preset, 'button', { ...restProps, type }));
```

Compute in a `$derived` and spread it (`{...buttonProps}`). Never inline `preset ?? …` in markup or hand-roll the spread — the helper keeps layering (atom spread → restProps → preset fallback) uniform.
...
`this.capability(...)` in the constructor (see `CollapsibleBondBase`); activate whole-bond via
`useCapabilities(bond)` + `setup()`. Use `createDisclosure(...)` + `disclosureCapability(...)`
for open/close components. Never sprinkle per-root `$effect`s in `*-root.svelte` for
focus/escape/animation — that pattern was deliberately removed.
...
- **Never spread a rest-props proxy inside a `$derived`** — it collapses fine-grained reactivity. The rest object is a pass-through / stable filtering Proxy, not a copy (`lifecycle-fast-path`).
...
Use `createFilter` (`src/lib/runes/filter.svelte.ts`) plus a keyed `{#each}` with Svelte transitions. **Derive the view; never mutate the source array.**
